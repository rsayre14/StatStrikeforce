...
def signup():
    ...
    r6Account = request.form.get('r6Account')

    if email == '1@yes.com' and password == '2' and r6Account == '3':
        success_message = f"User account created for: {email} and password: {password} and r6Account: {r6Account}."
        return jsonify({"success": True, "message": success_message}), 200
    else:
        invalid_message = f"Invalid Data"
        return jsonify({"success": False, "message": invalid_message}), 200
...
def login():
    ...
    if username == '1' and password == '2':
        success_message = f"Login successful for username: {username} and password: {password}."
        return jsonify({"success": True, "message": success_message}), 200
    else:
        invalid_message = f"Login invalid for username: {username} and password: {password}."
        return jsonify({"success": False, "message": invalid_message}), 200
        #note: status code 401 will trigger error message in formHandler.js
...
def generate_html():
    #takes the userinput from the 'Track here' search bar, consider data validation
    user_input = request.args.get('userinput')

    #hardcoded data, you can create variables from the database instead
    name = 'Spatie'
    WL = '3'
    HS = '47'
    KPR = '789'

    # Generates HTML code to be inserted into the '#searchresults' div, consider better formatting
    html_content = f"<div class='result-content'>Hello, {user_input}!</div><p>Name: {name}</p><p>WL: {WL}</p><p>HS: {HS}</p><p>KPR: {KPR}</p>"
    return html_content
